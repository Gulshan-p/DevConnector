  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })
      .then((profile) => {
        ...
        //map command will convert inthe form of dictionary
        if (removeIndex === -1) {
...
router.delete(
  ...
  (req, res) => {
    Profile.findOne({ user: req.user.id })
      .then((profile) => {
        //can add query profile not found
        // Get remove index
      ...
      .catch((err) => res.status(404).json({education: "education already deleted"})
      );
      //(err)); or can display just error
    }
);
